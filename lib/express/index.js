        ...
        function next(err) {
            ...
            if(err) {
                ...
                if (route.method == 'middleware') { 
                    ...
                    if (route.path == '/' ||
                        ...
                        if (route.hanlder.length == 4) {
                            route.hanlder(err, req, res, next);
                        } else {
                            next(err);
                        }
                    } else {
            ...
            if (route.method == 'middleware') { // 中间件
                ...
                if (route.path == '/' ||
                    ...
                    // 未匹配到路径则不执行中间件 hanlder，内部调用 next 继续向下执行
                    next();
                }
            } else { // 路由
